package de.schliemannosaurusrex.mukkeklopper.network
...
class NetworkGatekeeper(
    ...
    suspend fun canSync(): SyncDecision = withContext(Dispatchers.IO) {
        ...
        val onHomeNetwork = isHomeNetwork(settings)
        val vpnActive = if (!onHomeNetwork) isVpnActive() else false
        AppLog.d(TAG, "gate: homeNetwork=$onHomeNetwork vpnActive=$vpnActive requireVpn=${settings.requireVpnOutsideHome}")
        if (!onHomeNetwork && settings.requireVpnOutsideHome && !vpnActive) {
            AppLog.i(TAG, "sync blocked: not on home network and no VPN active")
            return@withContext SyncDecision.Blocked
        }
        verifyHostKey(settings).also { AppLog.d(TAG, "host key check result: $it") }
    }

    private fun isHomeNetwork(settings: MukkeSettings): Boolean =
    ...
    private suspend fun verifyHostKey(settings: MukkeSettings): SyncDecision {
        ...
        try {
            ...
            AppLog.w(TAG, "host key handshake failed for ${settings.serverHost}:${settings.serverPort}", e)
        ...
        return when {
            ...
            pinned == presented -> SyncDecision.Allowed
                ...
                AppLog.e(TAG, "HOST KEY MISMATCH for ${settings.serverHost}: pinned=$pinned presented=$presented")
                SyncDecision.MitmWarning(pinned = pinned, presented = presented)
            }
        }
    }

    private companion object {
        const val TAG = "NetworkGatekeeper"
