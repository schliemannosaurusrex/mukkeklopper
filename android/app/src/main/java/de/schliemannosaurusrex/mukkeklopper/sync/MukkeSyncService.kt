...
class MukkeSyncService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (intent?.action == ACTION_CANCEL) {
            AppLog.i(TAG, "sync cancel requested by user")
            SyncManager.cancel()
            return START_NOT_STICKY
        }

        AppLog.i(TAG, "sync foreground service starting")
        startForeground(NOTIFICATION_ID, buildNotification("Preparing sync…"))

        if (!SyncManager.isRunning) {
            val retryOnly = intent?.getStringArrayListExtra(SyncManager.EXTRA_RETRY_PATHS)
            scope.launch {
                SyncManager.runSync(applicationContext, interactive = true, retryOnly = retryOnly)
                stopSelf()
            }
            scope.launch {
    ...
    private fun buildNotification(text: String, progress: Int? = null): android.app.Notification {
        val cancelIntent = PendingIntent.getService(
            this,
            0,
            Intent(this, MukkeSyncService::class.java).setAction(ACTION_CANCEL),
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE,
        )
        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setSmallIcon(android.R.drawable.stat_sys_download)
            .setContentTitle("MukkeKlopper sync")
            .setContentText(text)
            .setOngoing(true)
    ...
    private companion object {
        const val TAG = "MukkeSyncService"
        const val CHANNEL_ID = "sync"
        const val NOTIFICATION_ID = 2
        const val ACTION_CANCEL = "de.schliemannosaurusrex.mukkeklopper.sync.CANCEL"
    }
}
